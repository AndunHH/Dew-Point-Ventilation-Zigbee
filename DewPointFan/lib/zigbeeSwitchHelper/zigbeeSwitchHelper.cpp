/* This helper class in mainly based on the Zigbee Switch example taken from:
https://github.com/espressif/arduino-esp32/tree/master/libraries/Zigbee/examples/Zigbee_On_Off_Switch
...
void ZigbeeSwitchHelper::printBoundDevicesLong() {
  // zbSwitch.printBoundDevices(Serial) does the ieee adress, but without naming the manufacturer
  // and light model. therefore hard coded long version

  std::list<zb_device_params_t *> boundLights = zbSwitch.getBoundDevices();
  for (const auto &device : boundLights) {
    /*Serial.printf("Device on endpoint %d, short address: 0x%x\r\n", device->endpoint,
    device->short_addr); Serial.printf( "IEEE Address: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\r\n",
