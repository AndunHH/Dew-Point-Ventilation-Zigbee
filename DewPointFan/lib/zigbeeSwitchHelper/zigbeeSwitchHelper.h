...
/// @brief ZigbeeSwitchHelper class to help with all zigbee related stuff. Call loop() regularly to check the status etc. Use setLightSetpoint(true/false) to turn on the zigbee light/socket.
...
public:
    ...
    ZigbeeSwitchHelper() : zbSwitch(ZigbeeSwitch(SWITCH_ENDPOINT_NUMBER)),
                           zigbeeSwitchHelperState(ZB_INIT),
                           lightSetpoint(false)
    {
    }
