...
enum VentilationUseFull {USEFULL, NODATA, NODATAINDOOR, NODATAOUTDOOR, TOOCOLDINSIDE, TOOCOLDOUTSIDE, INSIDEDRYENOUGH, OUTSIDENOTDRYENOUGH};

#define RING_BUFFER_SIZE 8 // Size of the ring buffer.

/// @brief ProcessSensorData class to read in two DHT sensors and calculate temperatur and humidities with a circularbuffer.
class ProcessSensorData {
    public: 
        ...
        ProcessSensorData() : 
            processSensorDataStates(INIT),
            condTempImin_degC(TEMP_I_MIN),
    ...
    private:
        ...
        boolean calculateAverage(CircularBuffer<TempAndHumidity, RING_BUFFER_SIZE> *buf, AvgMeasurement *avg);   
