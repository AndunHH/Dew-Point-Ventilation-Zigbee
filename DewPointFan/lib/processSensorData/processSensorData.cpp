...
/// @brief This is the loop function to read the temperature and humidity sensors and calculate wether ventilation is usefull or not
...
boolean ProcessSensorData::calculateAverage(CircularBuffer<TempAndHumidity, RING_BUFFER_SIZE> *buf, AvgMeasurement *avg)
{
  ...
  {
    if ((*buf)[i].temperature > 500 || (*buf)[i].humidity > 500 || isnan((*buf)[i].temperature) || isnan((*buf)[i].humidity))
    {
      // no valid data
...
void ProcessSensorData::createLogChar(char *logStr)
{
  ...
  snprintf(
      logStr, TEMPLOG_LENGTH,
      "%+3.1f;%+3.1f;%+3.1f;%+3.1f;%+3.1f;%+3.1f;%u;%u",
      avgMeasurementI.temperature,
      avgMeasurementO.temperature,
      avgMeasurementI.humidity,
      avgMeasurementO.humidity,
      avgMeasurementI.dewPoint,
